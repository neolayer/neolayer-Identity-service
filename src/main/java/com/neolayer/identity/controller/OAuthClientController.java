...
public class OAuthClientController {
        ...
        @PostMapping
        @Operation(summary = "Create OAuth Client", description = "Generate a new OAuth client with unique client ID and secret code for your application. "
                        +
                        "Optionally pass userId to associate with an existing user (defaults to user 1).")
        @ApiResponses(value = {
        ...
        public ResponseEntity<OAuthClientResponse> createOAuthClient(
                ...
                // Use userId from request body, default to 1 if not provided
        ...
        @Operation(summary = "List OAuth Clients", description = "Get all OAuth clients created by the authenticated user")
        @ApiResponses(value = {
        ...
        @GetMapping("/{id}")
        @PreAuthorize("hasAnyRole('USER', 'ADMIN')")
        @Operation(summary = "Get OAuth Client Details", description = "Retrieve detailed information for a specific OAuth client")
        @ApiResponses(value = {
        ...
        @PutMapping("/{id}")
        @PreAuthorize("hasAnyRole('USER', 'ADMIN')")
        @Operation(summary = "Update OAuth Client", description = "Update project details for an existing OAuth client")
        @ApiResponses(value = {
        ...
        @PreAuthorize("hasAnyRole('USER', 'ADMIN')")
        @Operation(summary = "Regenerate Client Secret", description = "Generate a new client secret. WARNING: This will invalidate the old secret immediately!")
        @ApiResponses(value = {
        ...
        @DeleteMapping("/{id}")
        @PreAuthorize("hasAnyRole('USER', 'ADMIN')")
        @Operation(summary = "Delete OAuth Client", description = "Remove an OAuth client. Applications using this client will no longer be authorized.")
        @ApiResponses(value = {
